...
val create : ('exe, 'lib) Package.Typed.t -> Build_dir.t -> ('exe, 'lib) t
val plan_exe : (true_t, _) t -> Build_plan.t
val plan_lib : (_, true_t) t -> Build_plan.t
val create_exe_plan : (true_t, _) Package.Typed.t -> Build_dir.t -> Build_plan.t
val create_lib_plan : (_, true_t) Package.Typed.t -> Build_dir.t -> Build_plan.t
